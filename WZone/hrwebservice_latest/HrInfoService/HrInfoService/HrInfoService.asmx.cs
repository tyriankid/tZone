...
namespace HrInfoService
{
    ...
    public class Service1 : System.Web.Services.WebService
    {
        ...
        [WebMethod(Description = "根据部门id获取所有子部门信息")]
        public DataSet GetChildrenDepByDepId(string depid) 
        {
            DataSet ds = null;
            try 
            {
                string sql = string.Format(@"select * from vBaseDepInfo where csr like '%{0}%'",depid);
                ds = db.GetDataSetBySQL(sql);
                return ds;
            } 
            catch (Exception ex) 
        ...
        [WebMethod(Description = "获取所有部门信息")]
        public DataSet GetDepInfo()
        {
            DataSet ds = null;
            try
            {
                string sql = string.Format(@"select depid,prowid,depidcsr,dbo.clip(depname,'/',1)depname,dbo.clip(depnamecsr,'/',1)depnamecsr,csr from vBaseDepInfo  
                                            WHERE (CHARINDEX( '/',depidcsr) <= 0 or CHARINDEX ('d098',depidcsr) > 0) AND CHARINDEX('/',csr) >0  AND CHARINDEX('组',dbo.clip(depname,'/',1)) <=0 AND depname NOT in ('管理费用','项目部') 
                                            ORDER BY prowid
                                            ");
                ds = db.GetDataSetBySQL(sql);
                return ds;
