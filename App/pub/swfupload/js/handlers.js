...
function AfterSelectFile() {
     if (fg_fileSizes>0) window.parent.document.getElementById("btnUp").disabled = false;
}
...
function fileQueueError(file, errorCode, message) {
    try {
        if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
            alert("抱歉, 你选择的文件个数超出最大限制.\n" + (message === 0 ? "" : "最多只允许选择【" + message + "】个文件"));
            return;
        }
...
function uploadSuccess(file, serverData) {
    try {
        ...
        upfile[upfile.length] = serverData;
    ...
    try {
        ...
        switch (errorCode) {
            case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:
                ...
                alert("[" + file.name + "]文件上传错误, 原因" + message);
                this.debug("Error Code: HTTP Error, File name: " + file.name + ", Message: " + message);
                break;
            case SWFUpload.UPLOAD_ERROR.UPLOAD_FAILED:
                //progress.setStatus("Upload Failed.");
                alert("[" + file.name + "]文件上传失败, 原因" + message);
...
function queueComplete(numFilesUploaded){
    ...
    window.parent.selectfilesize = fg_fileSizes;
    window.parent.closetype = "btnOK";
