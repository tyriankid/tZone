function _FormLoad() {
     ...
     jq1.Open(GetInitSql(true));
     jq2.Open(GetInitSql(false));
     //window.setTimeout("colorInit()",600);
     doChangeNow();
}
...
function doChangeNow(e){
    ...
    rowC1=jq1.RowCount();
    rowC2=jq2.RowCount();
    if($("#jq1_grid").find("tr") != null && $("#jq2_grid").find("tr") != null) //避免报错
    {
		/*2015-01-23 begin bugfix:当我的任务里为空时不执行变色的问题*/
		if(rowC2>0 && document.getElementById("jq1_pager_right").innerHTML.indexOf("无数据显示") >0 && rowId2!=$("#jq2_grid").find("tr")[1].id)
		{
...
function GetInitSql(isMine) {
    ...
    var sql="SELECT state,tasktype,typename,CASE WHEN LEN(needdate+' '+needhh+':'+needmm)>12 THEN (needdate+' '+needhh+':'+needmm)  END needdates,CASE WHEN LEN(appdate+' '+apphh+':'+appmm)>12 THEN (appdate+' '+apphh+':'+appmm)   END appdates,dbo.clip(a.AppDep,':',1)AppDeps,";
...
function GetQue(){
    if(jq2.item("state")==0)
    {
        Sql.AddCmd("update PUB_Task_Info set state=1,Producer='"+ uinfo +"' where rowid='"+ jq2.Item("rowid") +"'");
        Sql.AddCmd("insert into PUB_Log_Info(PubID,Message,LogUser,ObjID)Values('"+ jq2.item("rowid") +"','领取任务','"+ uname +"','"+ jq2.item("rowid") +"')");
        ExecSql(function(s){
            if(s=="")
            {
                alert("领取成功");
...
function SetState() {
    for(var i = 0; i < jq2.RowCount(); i++) {
    //alert(jq2.item("state",i+1));
        if (jq2.item("state",i+1) == "0"){
